...
def load_dynamic_contour(vertices, faces, contour_embeddings_path='None', static_embedding_path='None', angle=0):
    ...
    idx = []
    for i in range(len(lmk_face_idx_static)):
        idx.append(lmk_face_idx_static[i])
    for j in range(len(lmk_face_idx_dynamic)):
        idx.append(lmk_face_idx_dynamic[j])
    #total_lmks_faces = np.vstack([lmk_face_idx_static, lmk_face_idx_dynamic])
    total_lmks = np.vstack([dynamic_lmks, static_lmks])

    return total_lmks, np.array(idx)
...
def get_landmarks(vertices):
    ...
    total_lmks, landmarks_faces =load_dynamic_contour(vertices, faces, contour_embeddings_path=contour_embeddings_path, static_embedding_path=static_embedding_path, angle=int(angle))
    total_lmks=process_landmarks(total_lmks)
    return total_lmks, landmarks_faces
